...
**Adopt — perbaikan code complete, menunggu gateway restart.**

Fix logic benar dan minimal. `while True` + post-sleep break adalah ponytail solution terbaik: tidak menambah pre-sleep line, tidak mengubah loop semantics, hanya mengubah **kapan** exit check terjadi. Lebih simple daripada memindahkan spawn block 637 lines ke bawah.

**BUT: fix belum live.** Gateway yang running sekarang (PID 657515) loaded old code dari memory. Restart diperlukan untuk load fix.

## Risk

- **Gateway restart diperlukan** — tidak bisa dilakukan dari dalam gateway (security guard blocks self-restart). Butuh `hermes gateway restart` dari shell terpisah atau systemd timer.
...
- **Silent failure is the killer.** `_spawn_supervised` by design tidak restart clean exits — ini benar (anti busy-spin), tapi berarti watcher yang exit-on-spawn **tidang terdeteksi**. Future watchers harus punya health signal (log "watcher started" di first loop iteration).
...
1. **Restart gateway** untuk load fix — setelah restart, verify tracemalloc snapshot muncul dalam 31 menit
2. Setelah snapshot confirmed, mulai memory trend tracking (RSS + tracemalloc allocator data per 24h)
3. Pertimbangkan health-signal pattern untuk semua `_spawn_supervised` watchers
